...
export default function Pagination({links, from, to, total }) {
    ...
    if(!links[0].active) {
        ...
        prevButton = <button className={className + "rounded-l"}><TbArrowNarrowLeft className="mr-2 text-xl" /> Prev</button>
    }

    let nextButton
    if(!links[links.length - 1].active) {
        ...
        nextButton = <button className={className + "rounded-r"}>Next <TbArrowNarrowRight className="ml-2 text-xl" /></button>
    }
